...
const drawToDo = (toDoObj) => {
  ...
  if (toDoObj.done === false) {
      ...
      document.getElementById("pre").innerHTML = Number(document.getElementById("pre").innerHTML) + 1;
    }
    else {
        toDoListDone.appendChild(li);
        document.getElementById("done").innerHTML = Number(document.getElementById("pre").innerHTML) + 1;
    }
};
...
const toggleToDo = (event) => {
    ...
    if (!done) {
        ...
        toDoListDone.appendChild(li);
        document.getElementById("pre").innerHTML = Number(document.getElementById("pre").innerHTML) - 1;
        document.getElementById("done").innerHTML = Number(document.getElementById("done").innerHTML) + 1;
        indDone += 1;
    }
    else {
        toDoListDone.removeChild(li);
...
const saveListCache = () => {
    ...
    saveList.push(Number(document.querySelectorAll(".todo-list-pre > li > span").length));
    for (let toDoObj of document.querySelectorAll(".todo-list-pre > li > span")) {
        saveList.push(toDoObj['innerHTML'])
    }
    
    for (let toDoObj of document.querySelectorAll(".todo-list-done > li > span")) {
        saveList.push(toDoObj['innerHTML'])
    }
...
var DD = today.getDate()
    ...
    document.getElementById("date").innerHTML = YY + "년 " + MM + "월 " + DD + "일" + "  " + hh + ":" + mm;
};
